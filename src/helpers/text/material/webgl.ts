...
export class TextHelperWebglMaterial extends MeshBasicMaterial {
  atlas: TextHelperAtlas
  uniforms: TextUniforms

  constructor(uniforms: TextUniforms, atlas: TextHelperAtlas) {
    super({
      map: atlas.texture,
      transparent: true,
      alphaTest: .5,
    ...
    this.onBeforeCompile = shader => ShaderForge.with(shader)
      ...
      .vertex.replace('project_vertex', /* glsl */`
        ...
        gl_Position.z += -uTextDepthOffset;
  ...
  customProgramCacheKey() {
    return `TextHelperMaterial-${this.atlas.symbols}-${this.atlas.charGrid.x}-${this.atlas.charGrid.y}`
  }
}

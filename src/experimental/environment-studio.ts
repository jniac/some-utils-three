import { BackSide, BoxGeometry, Color, ColorRepresentation, CubeCamera, FloatType, Group, IcosahedronGeometry, Mesh, MeshBasicMaterial, MeshPhysicalMaterial, PlaneGeometry, PointLight, RGBAFormat, Scene, ShaderMaterial, SphereGeometry, Texture, TorusGeometry, UnsignedByteType, Vector3Tuple, WebGLCubeRenderTarget, WebGLRenderer } from 'three'
...
export class EnvironmentStudio {
   ...
   * ```
   * studio.createEnvironmentSphere({ map: texture1 })
   * studio.swapScenes()
   * studio.createEnvironmentCube({ map: texture2 })
  ...
  createEnvironmentSphere({
    ...
    const environmentMesh = setup(new Mesh(geometry, material), this.parts.scene)

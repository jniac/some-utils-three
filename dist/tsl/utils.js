...
// @ts-ignore
export const rotate2D = Fn(([vector, angle]) => {
    const v = vec2(vector);
    const a = float(angle);
...
// @ts-ignore
export const scale2D = Fn(([vector, scale, pivot]) => {
    const v = vec2(vector);
    const s = vec2(scale);
    const p = vec2(pivot);
...
export class InstancedStorage {
    ...
    constructor(countOrArray, type = 'vec3', typeSize = 3, defaultValue = [0, 0, 0]) {
        const createArray = () => {
 ...
 */
export function zOffset(object, zOffset = 1) {
...
export const rotationMatrix = Fn(([axis, angle]) => {
    ...
    const y = axis.y;
    const z = axis.z;
    return mat3(c.add(x.mul(x).mul(omc)), x.mul(y).mul(omc).sub(z.mul(s)), x.mul(z).mul(omc).add(y.mul(s)), x.mul(y).mul(omc).add(z.mul(s)), c.add(y.mul(y).mul(omc)), y.mul(z).mul(omc).sub(x.mul(s)), x.mul(z).mul(omc).sub(y.mul(s)), y.mul(z).mul(omc).add(x.mul(s)), c.add(z.mul(z).mul(omc)));
});
// @ts-ignore

...
import { isMatrix4, isObject3D, isVector3 } from '../is.js';
/**
 * NOTE:
 ...
 */
export function computePerspective(camera, container) {
...
function matrixResetRotation(matrix, resetScalar = 1) {
...
function matrixResetScale(matrix, scaleBase, resetScalar = 1) {
    const me = matrix.elements;
    _vector1.set(me[rx], me[ry], me[rz]);
...
function toMatrix4(source, optionalTarget) {
    if (isObject3D(source)) {
...
export function computeFrontFacing(camera, target) {
...
export function computeZIndex(camera, target) {
 ...
 */
export function computeMatrix3d(camera, target, container, pixelPerUnit = 100, resetRotation = 0, resetScale = 0, leftHanded = false) {
    const h = camera.projectionMatrix.elements[5];
...
export function updatePosition3d(container, div, camera, target, { pixelPerUnit = 100, resetRotation = 0, resetScale = 0, leftHanded = false, } = {}) {

...
import { Utils, _c0, _v0, _v1, _v2, _v3, _v4, _v5, _v6 } from './shared.js';
function fromOptionsDeclaration(arg) {
...
function computeArrowPoints(segments, arrowOptions) {
    ...
    for (let i = 0; i < segmentCounts; i++) {
        ...
        for (const arrowOption of arrowOptionsArray) {
            const { size = defaultArrowOptions.size, proportionalSize = defaultArrowOptions.proportionalSize, position = defaultArrowOptions.position, skip = defaultArrowOptions.skip, type = defaultArrowOptions.type, scale = defaultArrowOptions.scale, } = arrowOption ?? {};
            if (skip !== undefined) {
            ...
            for (const p of fromOneOrMany(position)) {
                const repeatCount = type === 'triple' ? 3 :
                    type === 'double' ? 2 :
                        1;
...
export class LinesManager extends BaseManager {
    ...
    segmentsArray(array, options) {
        ...
        const { key, color: colorArg, opacity: opacityArg, } = {
            ...defaultLineOptions,
            ...this.parts.defaults,
            ...options,
        };
    ...
    regularGrid(options) {
        const { color: colorArg, opacity: opacityArg, plane, size, subdivisions, } = { ...LinesManager.regularGridDefaults, ...options };

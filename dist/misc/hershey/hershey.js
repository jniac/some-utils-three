...
import { fromVector2Declaration } from 'some-utils-ts/declaration';
...
export function getHersheyPoints(text, { lineHeight = 1.25, height = Infinity, width = Infinity, align = .5, size = 1, } = {}) {
...
function getHelperPoints({ width, height, size }, textWidth, textHeight, pivotX, pivotY) {
    const points = [];
    let A, B, C, D;
    ...
    const d = .2 * size;
    points.push(new Vector3(-d, -d, 0), new Vector3(+d, +d, 0), new Vector3(+d, -d, 0), new Vector3(-d, +d, 0));
    return points;
}
 ...
 * ```
 */
export class HersheyText extends LineSegments {
    width;
    height;
    constructor(text, incomingProps, material) {
        ...
        const geometry = new BufferGeometry().setFromPoints(points)
            .translate(px, py, 0);
        super(geometry, material ?? new LineBasicMaterial({ vertexColors: true }));
        if (props.helper) {
